...
try
{
    WNDW.WNDW003 pu = new WNDW.WNDW003();
    pu.ShowDialog();
...
namespace WNDW
{
    ...
    /// <para>Msgs[6] = 제품코드 </para>
    /// </summary>
    
    public partial class WNDW003 : UIForm.FPCOMM1
    {
        #region 변수선언
        ...
        #region WNDW003 생성자
        public WNDW003(string Type)
        {
            strType = Type;
            InitializeComponent();
        }

        public WNDW003(string ProjectNo, string Type)
        {
            strType = Type;
            strProjNo = ProjectNo;
            InitializeComponent();
        ...
        public WNDW003(string ProjectNo, string Type, string Status)
        {
            strType = Type;
            strProjNo = ProjectNo;
            if (Status != "")
        ...
        public WNDW003(string ProjectNo, string ItemCd, string Type, string Status)
        {
            strType = Type;
            strProjNo = ProjectNo;
            strItemCd = ItemCd;
        ...
        private void WNDW003_Load(object sender, System.EventArgs e)
        {
            ...
            SystemBase.ComboMake.C1Combo(cboSTATUS, "usp_P_COMMON @pTYPE	= 'P040', @pLANG_CD = '" + SystemBase.Base.gstrLangCd + "', @pCOM_CD = 'P012', @pCO_CD = '" + SystemBase.Base.gstrCOMCD.ToString() + "'", 3); //진행상태
            ...
            if (strType == "S2")
            {
                txtMakeOrderNo.ReadOnly = true;
                txtMakeOrderNo.BackColor = SystemBase.Validation.Kind_Gainsboro;
                dtpMakeDtFr.Enabled = false;
                dtpMakeDtTo.Enabled = false;
                cboSTATUS.Visible = false; //진행상태
                c1Label24.Visible = false; //진행상태
            }
        ...
        #region 그리드조회
        private void Grid_search(string type, bool Msg)
        {
            ...
            if (SystemBase.Validation.GroupBox_SaveSearchValidation(groupBox1))
            {
                try
                {
                    ...
                    string strQuery = " usp_WNDW003 @pTYPE = '" + type + "'";
        ...
        private void fpSpread1_TopChange(object sender, FarPoint.Win.Spread.TopChangeEventArgs e)
        {
            ...
            if (e.NewTop >= ((AddRow * SDown) - FPHeight))
            {
                ...
                string strQuery = " usp_WNDW003 @pTYPE = '" + strType + "'";
                strQuery += ", @pPROJECT_NO = '" + txtProjectNo.Text + "' ";
                strQuery += ", @pPROJECT_NM = '" + txtProjectNm.Text + "' ";
                strQuery += ", @pITEM_CD = '" + txtItemCd.Text + "' ";
                strQuery += ", @pITEM_NM = '" + txtItemNm.Text + "' ";
        ...
        { if (e.KeyCode == Keys.Enter) Grid_search(strType, true); }
